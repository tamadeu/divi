...
serve(async (req) => {
  ...
  try {
    ...
    // Array para armazenar logs pendentes
    const pendingLogs: PendingAILog[] = []
    ...
    if (audio_data) {
      ...
      const transcriptionResult = await transcribeAudio(audio_data, audio_type || 'audio/webm', user.id, workspace_id, pendingLogs)
...
`
    ...
    if (geminiApiKey && !aiResult.success) {
      aiResult = await processWithGemini(prompt, transcribedText, user.id, workspace_id, pendingLogs)
    }

    // Tentar OpenAI se Gemini falhar
    const openaiApiKey = Deno.env.get('OPENAI_API_KEY')
    if (openaiApiKey && !aiResult.success) {
      aiResult = await processWithOpenAI(prompt, transcribedText, user.id, workspace_id, pendingLogs)
    }

    // Tentar Anthropic se outros falharem
    const anthropicApiKey = Deno.env.get('ANTHROPIC_API_KEY')
    if (anthropicApiKey && !aiResult.success) {
      aiResult = await processWithAnthropic(prompt, transcribedText, user.id, workspace_id, pendingLogs)
    }
    ...
    if (!aiResult.success) {
      ...
      // Adicionar log do fallback aos logs pendentes
      pendingLogs.push({
        user_id: user.id,
        workspace_id,
    ...
    return new Response(
      JSON.stringify({ 
        success: true, 
        transaction: aiResult.transaction,
        pending_logs: pendingLogs 
      }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    )
...
async function processWithGemini(prompt: string, inputText: string, userId: string, workspaceId: string, pendingLogs: PendingAILog[]) {
  ...
  try {
    ...
    if (response.ok && responseData.candidates?.[0]?.content?.parts?.[0]?.text) {
      ...
      // Adicionar log aos logs pendentes
      pendingLogs.push({
        user_id: userId,
    ...
    // Adicionar log de erro aos logs pendentes
    pendingLogs.push({
      user_id: userId,
      workspace_id: workspaceId,
      input_text: inputText,
...
async function processWithOpenAI(prompt: string, inputText: string, userId: string, workspaceId: string, pendingLogs: PendingAILog[]) {
  const startTime = Date.now()
  const openaiApiKey = Deno.env.get('OPENAI_API_KEY')!
...
async function processWithAnthropic(prompt: string, inputText: string, userId: string, workspaceId: string, pendingLogs: PendingAILog[]) {
  const startTime = Date.now()
  const anthropicApiKey = Deno.env.get('ANTHROPIC_API_KEY')!
...
async function transcribeAudio(audioBase64: string, audioType: string, userId: string, workspaceId: string, pendingLogs: PendingAILog[]) {
  const startTime = Date.now()
  const openaiApiKey = Deno.env.get('OPENAI_API_KEY')
  ...
  try {
    ...
    if (response.ok) {
      ...
      // Adicionar log de transcrição aos logs pendentes
      pendingLogs.push({
        user_id: userId,
        workspace_id: workspaceId,
    ...
    // Adicionar log de erro de transcrição aos logs pendentes
    pendingLogs.push({
      user_id: userId,
      workspace_id: workspaceId,
